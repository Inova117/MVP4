...
import { getDataSource, type DataSourceType } from '@/lib/mock-data/data-source-registry'
...
function analyzeGoogleAnalytics(data: any): Insight[] {
    const insights: Insight[] = []

    // Calculate trends
    const recentMetrics = data.dailyMetrics.slice(-7)
    const avgUsers = recentMetrics.reduce((sum: number, d: any) => sum + d.activeUsers, 0) / recentMetrics.length
    const avgConversionRate = recentMetrics.reduce((sum: number, d: any) => sum + d.conversionRate, 0) / recentMetrics.length
    ...
    insights.push({
        ...
        description: 'Your organic search traffic accounts for 42% of total visitors, which is above industry average (35%). This indicates strong SEO performance.',
    ...
    if (recentMetrics[6].bounceRate > 0.45) {
        insights.push({
            ...
            description: 'Bounce rate is higher than optimal (45%+). Users may not be finding what they expect on landing pages.',
            metric: 'Bounce Rate',
            value: `${(recentMetrics[6].bounceRate * 100).toFixed(1)}%`,
            action: 'Review top landing pages and improve content relevance, page load speed, and call-to-actions',
            priority: 'high'
        })
    }
    ...
    insights.push({
        ...
        description: 'Mobile traffic represents 35% of visitors but has 20% lower conversion rate than desktop. This gap suggests room for mobile UX improvements.',
        metric: 'Mobile Conversion Gap',
        value: '-20%',
        action: 'Conduct mobile usability audit and optimize checkout flow for mobile devices',
        priority: 'medium'
    })
    ...
    if (avgConversionRate > 0.03) {
        insights.push({
            ...
            action: 'Document and replicate successful patterns across other campaigns',
            priority: 'medium'
        })
    }
...
function analyzeMetaAds(data: any): Insight[] {
    const insights: Insight[] = []

    const campaigns = data.campaigns
    const bestCampaign = campaigns.reduce((best: any, camp: any) =>
        camp.roas > best.roas ? camp : best, campaigns[0]
    )
    ...
    insights.push({
        ...
        description: 'Video creatives have 23% higher CTR than static images. The product demo video is your top performer.',
    ...
    insights.push({
        ...
        description: 'The 25-34 age demographic shows highest conversion rate (38% of conversions) but only receives 30% of ad spend. Rebalance budget allocation.',
...
function analyzeInternalSaaS(data: any): Insight[] {
    const insights: Insight[] = []

    const recentMetrics = data.dailyMetrics.slice(-7)
    const avgChurn = recentMetrics.reduce((sum: number, d: any) => sum + d.churnRate, 0) / recentMetrics.length
    ...
    insights.push({
        ...
        description: 'Monthly Recurring Revenue grew 12% this month, driven by new Enterprise customers and Professional tier upgrades.',
        metric: 'MRR Growth',
        value: '+12%',
        change: 12,
        action: 'Focus sales efforts on mid-market companies (50-500 employees) showing highest conversion',
        priority: 'high'
    })
    ...
    if (avgChurn > 0.025) {
        insights.push({
            ...
            action: 'Implement onboarding email sequence and in-app feature tours for new users',
            priority: 'high'
        })
    }
    ...
    insights.push({
        ...
        description: 'Only 19% of Professional/Enterprise users utilize API access, despite this being a key differentiator. Many may not know it exists.',
        metric: 'API Adoption',
        value: '19%',
        action: 'Create API quick-start guides and showcase integration examples in dashboard',
        priority: 'medium'
    })
    ...
    insights.push({
        ...
        description: '234 Starter plan users have exceeded their monthly limits 3+ times. They\'re likely ready for Professional tier.',
        metric: 'Upgrade Candidates',
        value: '234 users',
        action: 'Send targeted upgrade campaign with 20% discount for annual commitment',
        priority: 'high'
    })
...
function analyzeEcommerce(data: any): Insight[] {
    const insights: Insight[] = []
    ...
    insights.push({
        ...
        description: 'Weekend sales are 30% higher than weekdays, suggesting strong B2C leisure shopping patterns. Saturday is your highest-performing day.',
    ...
    // Cart Abandonment
    const avgAbandonment = data.dailyMetrics.slice(-7).reduce((sum: number, d: any) =>
        sum + d.cartAbandonment, 0) / 7
    ...
    insights.push({
        ...
        action: 'Implement guest checkout and show shipping costs earlier in funnel',
        priority: 'high'
    })
    ...
    insights.push({
        ...
        description: 'Electronics generates 32% of revenue with highest average order value ($118). "Wireless Headphones Pro" alone drives 10% of total sales.',
        metric: 'Electronics Revenue',
        value: '32%',
        action: 'Expand electronics catalog and create bundle deals with top products',
        priority: 'high'
    })
    ...
    insights.push({
        ...
        description: '58% repeat purchase rate is excellent (industry avg: 30%). Your loyalty program and email marketing are working well.',
    ...
    insights.push({
        ...
        description: 'Average order value is $92, but free shipping starts at $100. Consider lowering threshold to $85 to boost conversions.',
...
export function getHealthScore(sourceId: DataSourceType): {
    ...
    if (score >= 85) {
        status = 'excellent'
        summary = 'Your metrics are performing exceptionally well. Keep up the momentum!'
    } else if (score >= 70) {
        status = 'good'
        summary = 'Overall healthy performance with some opportunities for optimization.'
    } else if (score >= 50) {
