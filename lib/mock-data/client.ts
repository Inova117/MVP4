...
export const mockDataClient: DataClient = {
    ...
    async getDashboard(id: string): Promise<Dashboard | null> {
        const dashboard = dashboards.find((d) => d.id === id && d.user_id === mockUser.id)
    ...
    async updateDashboard(id: string, data: Partial<Dashboard>): Promise<Dashboard> {
        const index = dashboards.findIndex((d) => d.id === id && d.user_id === mockUser.id)
        if (index === -1) {
    ...
    async deleteDashboard(id: string): Promise<void> {
        dashboards = dashboards.filter((d) => !(d.id === id && d.user_id === mockUser.id))
    ...
    async updateAlert(id: string, data: Partial<Alert>): Promise<Alert> {
        const index = alerts.findIndex((a) => a.id === id && a.user_id === mockUser.id)
        if (index === -1) {
